    ...
    private void buscarVendedor() {
        ...
        if (!documento.equals("")) {
            ...
            if (respuestaFindVendedor != null) {
                ...
                JsonObject registroVendedor = parser.parse(respuestaFindVendedor).getAsJsonObject();
                JsonArray vendedores = registroVendedor.getAsJsonArray("vendedor");

                if (vendedores.size() > 0) {
                    for (int i = 0; i < vendedores.size(); i++) {
                        JsonObject vendedor = vendedores.get(i).getAsJsonObject();
                        String estado = vendedor.get("estado").getAsString();

                        if (estado.equals("1")) {
                            String nombreVendedor = vendedor.get("nombre").getAsString();
                            String correo = vendedor.get("correo").getAsString();
                            String rol = vendedor.get("rol").getAsString();

                            etq_rol_vendedor.setForeground(Color.black);
                            etq_rol_vendedor.setHorizontalAlignment(javax.swing.SwingConstants.LEADING);
                            etq_nombres_vendedor.setText("Nombre: " + nombreVendedor);
                ...
                System.out.println("Vendedor: " + registroVendedor);
            } else {
