...
public class EditarVendedor extends javax.swing.JPanel {
    ...
    public void ObtenerDatosVendedor(){
        ...
        try {
            ...
            JsonArray registros = jsonObject.getAsJsonArray("vendedor");

            for (JsonElement registro : registros) {
                JsonObject registrosOb = registro.getAsJsonObject();

                String identidad = registrosOb.get("documento").getAsString();
                String nombre = registrosOb.get("nombre").getAsString();
                String correo = registrosOb.get("correo").getAsString();
                String passw = registrosOb.get("contrasenia").getAsString();
                String estados = registrosOb.get("estado").getAsString();
                if (estados.equals("1")) {
                    estado = "ACTIVO";
